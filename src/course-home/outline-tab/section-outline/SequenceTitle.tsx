    ...
    <div className="row w-100 m-0">
      <div className="col-auto p-0">
        {complete ? (
          <Icon
            src={CheckCircle}
            className="float-left v2-text-black-600 mt-1"
      ...
      <div className="col-10 p-0 ml-3 text-break text-base d-flex align-items-center">
        <span className="align-middle">{displayTitle}</span>
        <span className="sr-only">
          , {intl.formatMessage(complete ? messages.completedAssignment : messages.incompleteAssignment)}
        </span>
        <EffortEstimate className="ml-3 align-middle badge-activity" block={sequence} />
      </div>
    </div>
  );
};
